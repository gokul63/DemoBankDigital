Feature: Digital Bank
  ...
  Scenario Outline: To verify application is able to do fund transfer
    ...
    Examples: 
      | userName       | password    | senderAccount | receiverAccount | amount |
      | goku@gmail.com | Gokulmsd007 |           166 |             180 |   10.0 |
	
	
	@test2
  Scenario Outline: To verify the application is able to perform deposit into account
 		When User enters the "<userName>" and "<password>" to log in to application
    When User enters the new  "<userName>" and new"<password>" to log in to application
    ...
    Examples: 
      | userName      | password | accountForDeposit | depositAmount |
      | ash@gmail.com | Bank@123 |               293 |            20 |
  ...
  Scenario Outline: To verify user can able to perform Saving account opening
    ...
    #Then User can able to view the newly created account with "<AccountName>" successfully
    #And User logs out of the application

    Examples: 
      | userName       | password    | AccountName | DepositAmount |
      | goku@gmail.com | Gokulmsd007 | Sanjana     |           100 |
